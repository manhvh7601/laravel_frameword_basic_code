...
@section('title', 'ManhVH')
@section('content')
<!-- HERO SECTION-->
<div class="container">
    <section class="hero pb-3 bg-cover bg-center d-flex align-items-center"
        style="background: url({{ asset('style/img/hero-banner-alt.jpg') }})">
                    ...
                    <div class="product text-center">
                        <div class="position-relative mb-3">
                            <div class="badge text-white badge-"></div><a class="d-block"
                                href="{{ route('detail', ['id' => $product->id]) }}"><img class="img-fluid w-100"
                                    src="{{ asset('upload/' . $product->image) }}" alt="..."></a>
                            <div class="product-overlay">
                                <ul class="mb-0 list-inline">
                                    @auth
                                        <li class="list-inline-item m-0 p-0"><a class="btn btn-sm btn-dark"
                                                href="{{ route('addCart', ['id' => $product->id]) }}">Add to cart</a></li>
                                    @endauth
                                </ul>
                            </div>
                        </div>
                        <h6> <a class="reset-anchor"
                                href="{{ route('detail', ['id' => $product->id]) }}">{{ $product->name }}</a></h6>
